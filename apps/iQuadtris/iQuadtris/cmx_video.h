...
namespace cmx {
	...
	class Surface {
		...
		inline void copySurfaceToBuffer(unsigned int *buf, int width, int height, cmx::video::Surface &surf, Rect src_rect, Rect dst_rect) {
            
            CGRect rp = CGRectMake(dst_rect.x, dst_rect.y, dst_rect.w, dst_rect.h);
            [surf.image_source drawInRect: rp];
            /*
