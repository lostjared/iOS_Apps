...
namespace cmx {
	namespace video {
		...
		void Surface::saveImage(std::string image) {
            ...
            colorSpace = CGBitmapContextGetColorSpace(cgctx);
            context = CGBitmapContextCreateWithData(data,w,h,CGBitmapContextGetBitsPerComponent(cgctx), CGBitmapContextGetBytesPerRow(cgctx), colorSpace, CGBitmapContextGetBitmapInfo(cgctx), nil, nil);
            */
		...
		int Surface::loadImage(std::string image) {
            ...
            NSString *str = [NSString stringWithUTF8String: image.c_str()];
            image_source = [UIImage imageNamed:str];
            if(image_source) return 1;
            /*
            CGImage *inImage = [image_source CGImage];
            CGContextRef cgctx = CreateARGBBitmapContext(inImage);
            if (cgctx == NULL)
            {
                return 0;
            ...
            CGRect rect = CGRectMake(0,0,(float)w,(float)h);
            CGContextDrawImage(cgctx, rect, inImage);
            void *data = CGBitmapContextGetData (cgctx);
            if (data != NULL) {
                createSurface(w,h,4);
                unsigned int index = 0, total = w * h * 4;
                while((index < total)) {
                    unsigned int *buf = (unsigned int *)this->buffer;
                    unsigned int *dat = (unsigned int *)data;
                    buf += index;
                    dat += index;
                    *buf = *dat;
                    unsigned char *to = (unsigned char*)buf, *frm = (unsigned char*)dat;
                    buf[0] = dat[1];
                    buf[1] = dat[2];
                    buf[2] = dat[3];
                    buf[3] = dat[0];
